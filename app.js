const express = require('express');
const bodyParser = require('body-parser');
const session = require('express-session');

const app = express();
const port = 3000;

// Set EJS as the view engine
app.set('view engine', 'ejs');

// Middleware configuration
app.use(bodyParser.json());
app.use(bodyParser.urlencoded({ extended: true }));

app.use(session({
  secret: 'your-secret-key',
  resave: false,
  saveUninitialized: true
}));

// Sample users for authentication
const users = [
  { id: 1, username: 'user1', password: 'password1' },
  { id: 2, username: 'user2', password: 'password2' }
];

// Middleware to authenticate user
const authenticateUser = (username, password) => {
  return users.find((user) => user.username === username && user.password === password);
};

// Middleware to check if the user is logged in
const requireLogin = (req, res, next) => {
  if (!req.session.userId) {
    return res.status(401).send('You must be logged in to access this resource');
  }
  next();
};

// Routes
app.get('/login', (req, res) => {
  res.render('login');
});

app.post('/login', (req, res) => {
  const { username, password } = req.body;

  // Simulate authentication
  const authenticatedUser = authenticateUser(username, password);

  if (authenticatedUser) {
    // Set the user ID and login time in the session
    req.session.userId = authenticatedUser.id;
    req.session.loginTime = new Date();
    console.log("req.session:", req.session);
    console.log("authenticatedUser:", authenticatedUser);
    res.redirect('/dashboard');
  } else {
    console.log("authenticatedUser:", authenticatedUser);
    res.status(401).send('Invalid username or password');
  }
});

app.get('/dashboard', requireLogin, (req, res) => {
  res.render('dashboard', { userId: req.session.userId });
});

app.post('/updateState', requireLogin, (req, res) => {
  const newState = req.body.state; // 'yes' or 'no'
  const changedBy = req.session.userId;
  const changeTime = new Date();

  // Update the state in your application logic
  // For this example, we are pushing the state change to an array
  // stateChanges.push({ state: newState, changedBy, changeTime });

  res.render('nextpagetologin'); // Render the nextpagetologin.ejs view
});

app.get('/logout', (req, res) => {
  // Destroy the session on logout
  req.session.destroy((err) => {
    if (err) {
      console.error(err);
      return res.status(500).send('Internal Server Error');
    }
    res.send('Logout successful');
  });
});

app.get('/activeSessions', (req, res) => {
  const activeSessions = [];

  // Iterate over all active sessions in the store
  app._router.stack.forEach((middleware) => {
    if (middleware.route && middleware.route.path === '/dashboard') {
      // Collect information about active sessions
      const sessions = req.sessionStore.sessions;
      for (const key in sessions) {
        const sessionData = JSON.parse(sessions[key]);
        activeSessions.push({
          userId: sessionData.userId,
          loginTime: sessionData.loginTime,
        });
      }
    }
  });

  res.render('activesessions', { sessions: activeSessions });
});

// Start the server
app.listen(port, () => {
  console.log(`Server is running on port ${port}`);
});
